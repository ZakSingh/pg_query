...
// A deparse state part is all text associated to a major keyword
typedef struct DeparseStatePart
{
	const char *keyword;
	List *subparts; // List of StringInfo items (one per line to be emitted in pretty print mode)
	bool indent_subparts;
} DeparseStatePart;

typedef struct DeparseStateStatement
{
	List *parts; // List of DeparseStatePart items

	struct DeparseStateStatement *parent;
} DeparseStateStatement;

typedef struct DeparseState
{
	DeparseStateStatement *current;

	StringInfo str;
	PostgresDeparseOpts opts;

	/*
	 * Index into the next comment to be checked for insertion into the deparse
	 * output (if this exceeds the number of comments, it is ignored)
	 */
...
{
	DeparseStateStatement *stmt = state->current;
	if (!stmt->parts)
	{
		DeparseStatePart *part = palloc0(sizeof(DeparseStatePart));
		part->subparts = lappend(part->subparts, palloc0(sizeof(StringInfoData)));
		initStringInfo((StringInfo) llast(part->subparts));
		stmt->parts = lappend(stmt->parts, part);
	}
	DeparseStatePart *part = (DeparseStatePart *) llast(stmt->parts);
	StringInfo subpart = (StringInfo) llast(part->subparts);
	Assert(subpart != NULL);
	return subpart;
}
...
static void
deparseAppendBreakpoint(DeparseState *state)
{
	DeparseStateStatement *stmt = state->current;
	DeparseStatePart *cur = (stmt && stmt->parts) ? (DeparseStatePart *) llast(stmt->parts) : NULL;
	if (cur)
	{
		StringInfo s = makeStringInfo();
		cur->subparts = lappend(cur->subparts, s);
	}
	else
	{
...
static void
deparseAppendMajorKeyword(DeparseState *state, const char *keyword, bool indent_subparts)
{
	DeparseStateStatement *stmt = state->current;
	DeparseStatePart *part = palloc0(sizeof(DeparseStatePart));

	if (list_length(stmt->parts) > 0)
		removeTrailingSpace(state);

	part->keyword = keyword;
	part->subparts = lappend(part->subparts, makeStringInfo());
	part->indent_subparts = indent_subparts;
...
{
	if (state->next_comment_index + 1 > state->opts.comment_count)
		return;

	PostgresDeparseComment *comment = state->opts.comments[state->next_comment_index];
	if (comment->after_location <= location)
	{
		StringInfo str = state->str;
		if (state->current)
			str = deparseGetCurrentStringInfo(state);

		if (comment->newline_before_comment)
		{
			if (state->current && state->opts.pretty_print)
			{
				if (deparseGetCurrentStringInfo(state)->len != 0)
		...
		if (state->current) /* refresh to handle breakpoint */
			str = deparseGetCurrentStringInfo(state);
		appendStringInfoString(str, comment->str);

		if (comment->newline_after_comment)
		{
			if (state->current && state->opts.pretty_print)
				deparseAppendBreakpoint(state);
			else
		...
		state->next_comment_index++;
	}
}

static void
deparseEmit(DeparseState *state)
{
	ListCell *lc;
	ListCell *lc2;
	DeparseStateStatement *stmt = state->current;

	Assert(stmt->parent == NULL); // This must be the top-level statement

	foreach (lc, stmt->parts)
	{
		DeparseStatePart *part = (DeparseStatePart *) lfirst(lc);
		bool first_part = list_cell_number(stmt->parts, lc) == 0;
		bool single_line = list_length(part->subparts) == 1;

		if (part->keyword != NULL)
		{
			if (!first_part)
			{
				removeTrailingSpaceFromStr(state->str);
				if (state->opts.pretty_print)
					appendStringInfoChar(state->str, '\n');
				else
					appendStringInfoChar(state->str, ' ');
			}
			appendStringInfo(state->str, "%s", part->keyword);
		}
		foreach (lc2, part->subparts)
		{
			StringInfo s = (StringInfo) lfirst(lc2);
			bool first_subpart = list_cell_number(part->subparts, lc2) == 0;
			if (state->opts.pretty_print && !single_line && !(first_subpart && !part->indent_subparts))
				appendStringInfoChar(state->str, '\n');
			else if (!first_subpart || part->keyword != NULL)
				appendStringInfoChar(state->str, ' ');
			if (state->opts.pretty_print && !single_line && part->indent_subparts)
			{
				for (int i = 0; i < state->opts.indent_size; i++)
					appendStringInfoChar(state->str, ' ');
			}
			appendStringInfoString(state->str, s->data);
			removeTrailingSpaceFromStr(state->str);

			pfree(s->data);
			pfree(s);
		}
		list_free(part->subparts);
		pfree(part);
	}
	list_free(stmt->parts);
	stmt->parts = NIL;
}

static void
deparseStatePushStatement(DeparseState *state)
{
	DeparseStateStatement *stmt = palloc0(sizeof(DeparseStateStatement));
	stmt->parent = state->current;
	state->current = stmt;
}

static void
deparseStatePopStatement(DeparseState *state)
{
	ListCell *lc;
	ListCell *lc2;
	DeparseStateStatement *stmt = state->current;
	DeparseStatePart *parent_part;
	Assert(stmt != NULL);

	// If its the top-level statement, emit instead
	if (stmt->parent == NULL)
	{
		deparseEmit(state);
		return;
	...
	// In case parent doesn't have any parts (e.g. UNION statement), add a dummy part for us to write to
	if (!stmt->parent->parts)
	{
		DeparseStatePart *part = palloc0(sizeof(DeparseStatePart));
		part->subparts = lappend(part->subparts, makeStringInfo());
		stmt->parent->parts = lappend(stmt->parent->parts, part);
	}

	parent_part = (DeparseStatePart *) llast(stmt->parent->parts);
	Assert(parent_part != NULL);
	size_t prefix_len = state->opts.indent_size; //((StringInfo) llast(parent_part->subparts))->len;

	state->current = stmt->parent;

	// Step through all lines, as if we did the final emit, but instead emit line-by-line into the parent
	// If pretty printing, prefix it with the length of the last line in the parent
	foreach (lc, stmt->parts)
	{
		DeparseStatePart *part = (DeparseStatePart *) lfirst(lc);
		bool first_part = list_cell_number(stmt->parts, lc) == 0;
		...
		{
			StringInfo s = makeStringInfo();
			appendStringInfoString(s, part->keyword);
			if (single_line)
			{
				StringInfo line = (StringInfo) llast(part->subparts);
				if (line->len > 0)
					appendStringInfo(s, " %s", line->data);
				llast(part->subparts) = s;
				pfree(line->data);
				pfree(line);
			}
			else
			{
				part->subparts = list_insert_nth(part->subparts, 0, s);
			}
		}
		...
		foreach (lc2, part->subparts)
		{
			...
			if (!state->opts.pretty_print && first_part && first_subpart && list_length(parent_part->subparts) > 0)
			{
				target = (StringInfo) llast(parent_part->subparts);
			}
			else
			{
				target = makeStringInfo();
				parent_part->subparts = lappend(parent_part->subparts, target);
			}

			if (state->opts.pretty_print)
			{
				size_t len = prefix_len;
				if (!first_subpart) // Not a major keyword
					len += state->opts.indent_size;
				for (int i = 0; i < len; i++)
					appendStringInfoChar(target, ' ');
			}

			appendStringInfoString(target, s->data);
	...
	if (state->opts.pretty_print)
		parent_part->subparts = lappend(parent_part->subparts, makeStringInfo());
}
...
static void deparseFromClause(DeparseState *state, List *l)
{
	if (list_length(l) > 0)
	{
		deparseAppendMajorKeyword(state, "FROM", true);
...
static void deparseWhereClause(DeparseState *state, Node *node)
{
	if (node != NULL)
	{
		deparseAppendMajorKeyword(state, "WHERE", true);
		deparseExpr(state, node, DEPARSE_NODE_CONTEXT_A_EXPR);
		deparseAppendStringInfoChar(state, ' ');
	}
}
...
static void deparseOptSortClause(DeparseState *state, List *l)
{
	...
	if (list_length(l) > 0)
	{
		deparseAppendMajorKeyword(state, "ORDER BY", true);
...
static void deparseSelectStmt(DeparseState *state, SelectStmt *stmt)
{
	...
	switch (stmt->op) {
		case SETOP_NONE:
			...
			deparseAppendMajorKeyword(state, "SELECT", true);
			...
			{
				deparseAppendMajorKeyword(state, "GROUP BY", true);
			...
			{
				deparseAppendMajorKeyword(state, "HAVING", true);
			...
			{
				deparseAppendMajorKeyword(state, "WINDOW", true);
	...
	if (stmt->limitCount != NULL)
	{
		if (stmt->limitOption == LIMIT_OPTION_COUNT)
			deparseAppendMajorKeyword(state, "LIMIT", true);
	...
	{
		deparseAppendMajorKeyword(state, "OFFSET", true);
...
void deparseRawStmt(StringInfo str, struct RawStmt *raw_stmt)
{
	...
	state->str = str;
	// state->opts is zero-initialized (pretty print = false)
...
static void deparseWithClause(DeparseState *state, WithClause *with_clause)
{
	...
	deparseAppendMajorKeyword(state, "WITH", false);
	...
	switch (join_expr->jointype)
	{
		case JOIN_INNER: /* matching tuple pairs only */
			if (!join_expr->isNatural && join_expr->quals == NULL && list_length(join_expr->usingClause) == 0)
				deparseAppendMajorKeyword(state, "CROSS JOIN", true);
			else if (join_expr->isNatural)
				deparseAppendMajorKeyword(state, "NATURAL JOIN", true);
			else
				deparseAppendMajorKeyword(state, "JOIN", true);
			break;
		case JOIN_LEFT: /* pairs + unmatched LHS tuples */
			if (join_expr->isNatural)
				deparseAppendMajorKeyword(state, "NATURAL LEFT JOIN", true);
			else
				deparseAppendMajorKeyword(state, "LEFT JOIN", true);
			break;
		case JOIN_FULL: /* pairs + unmatched LHS + unmatched RHS */
			if (join_expr->isNatural)
				deparseAppendMajorKeyword(state, "NATURAL FULL JOIN", true);
			else
				deparseAppendMajorKeyword(state, "FULL JOIN", true);
			break;
		case JOIN_RIGHT: /* pairs + unmatched RHS tuples */
			if (join_expr->isNatural)
				deparseAppendMajorKeyword(state, "NATURAL RIGHT JOIN", true);
			else
				deparseAppendMajorKeyword(state, "RIGHT JOIN", true);
			break;
		case JOIN_SEMI:
		case JOIN_ANTI:
...
static void deparseCaseExpr(DeparseState *state, CaseExpr *case_expr)
{
	...
	{
		deparseAppendMajorKeyword(state, "ELSE", true);
...
static void deparseCaseWhen(DeparseState *state, CaseWhen *case_when)
{
	deparseAppendMajorKeyword(state, "WHEN", true);
...
static void deparseInsertStmt(DeparseState *state, InsertStmt *insert_stmt)
{
	...
	deparseAppendMajorKeyword(state, "INSERT", true);
	deparseAppendStringInfoString(state, "INTO ");
...
static void deparseUpdateStmt(DeparseState *state, UpdateStmt *update_stmt)
{
	...
	deparseAppendMajorKeyword(state, "UPDATE", true);
...
static void deparseMergeStmt(DeparseState *state, MergeStmt *merge_stmt)
{
	...
	deparseAppendMajorKeyword(state, "MERGE", true);
	deparseAppendStringInfoString(state, "INTO ");
...
static void deparseDeleteStmt(DeparseState *state, DeleteStmt *delete_stmt)
{
	...
	deparseAppendMajorKeyword(state, "DELETE", true);
	deparseAppendStringInfoString(state, "FROM ");
...
static void deparseExplainStmt(DeparseState *state, ExplainStmt *explain_stmt)
{
	...
	deparseAppendMajorKeyword(state, "EXPLAIN", true);
...
static void deparseCopyStmt(DeparseState *state, CopyStmt *copy_stmt)
{
	...
	deparseAppendMajorKeyword(state, "COPY", true);
...
static void deparseStmt(DeparseState *state, Node *node)
{
	...
	// SelectStmt push/pop is handled directly in deparseSelectStmt, since it can show up nested
	if (!skip_push_pop)
		deparseStatePopStatement(state);
}
